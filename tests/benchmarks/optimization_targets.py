...
class OptimizationBenchmarkSuite:
        ...
        for test_case in test_cases:
            for num_samples in [1000, 5000, 10000]:
                ...
                result = self._run_single_benchmark(
                    f"simple_2d_{test_case['name']}_s{num_samples}",
                    test_case,
                    config
                )
    ...
    def _benchmark_high_dimensional(self):
        ...
        test_case = {
            'name': '5d_linear',
            'certificate': ' + '.join([f'{var}**2' for var in ['x1', 'x2', 'x3', 'x4', 'x5']]) + ' - 3.0',
            'system': {
                'variables': ['x1', 'x2', 'x3', 'x4', 'x5'],
                'dynamics': [f'-{var}' for var in ['x1', 'x2', 'x3', 'x4', 'x5']],
                'initial_set_conditions': [' + '.join([f'{var}**2' for var in ['x1', 'x2', 'x3', 'x4', 'x5']]) + ' <= 1.0'],
                'unsafe_set_conditions': [' + '.join([f'{var}**2' for var in ['x1', 'x2', 'x3', 'x4', 'x5']]) + ' >= 16.0'],
                'safe_set_conditions': [],
    ...
    def _run_single_benchmark(self, 
                            ...
                            config: DictConfig) -> BenchmarkResult:
    ...
    def _evaluate_targets(self):
        ...
        simple_2d_times = [r.validation_time for r in self.results 
            ...
            for target in self.targets:
                if target.name == "fast_validation":
                    ...
                    target.improvement_percentage = (
                        ...
                        if avg_time > target.target_value else 100
                    )
    ...
    def _generate_report(self):
        ...
        for target in self.targets:
            ...
            print(f"  Current: {target.current_value:.3f}" if target.current_value else "  Current: Not measured")
...
def main():
    ...
    parser.add_argument('--output', default='optimization_benchmarks',
