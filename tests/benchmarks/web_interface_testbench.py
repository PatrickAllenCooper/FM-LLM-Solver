...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('testbench.log')
    ]
)
    ...
    def test_bounds_creation(self) -> TestResult:
        ...
        def _test():
            ...
            for system in test_systems:
                ...
                parsed_system = self.verification_service.parse_system_description(system['description'])
            ...
            successful_bounds = sum(1 for r in results.values() if r['bounds_created'] and r['all_variables_bounded'])
        ...
        def _test():
            ...
            for scenario in conversation_scenarios:
                ...
                results[scenario['name']] = {
                    ...
                    'correct_prediction': (extracted is not None) == scenario['expected_extraction'],
    ...
    def test_domain_bounds_extraction(self) -> TestResult:
        ...
        def _test():
            ...
            for i, message in enumerate(test_messages):
                ...
                extracted_bounds = self.conversation_service._extract_domain_bounds_from_conversation(
                    mock_conversation
                )
                
                results[f'message_{i}'] = {
                    'bounds_extracted': extracted_bounds is not None and len(extracted_bounds) > 0,
                    'x_bound_correct': extracted_bounds.get('x') == [-2, 2] if extracted_bounds else False,
                    'y_bound_correct': extracted_bounds.get('y') == [-1, 1] if extracted_bounds else False,
                    'extracted_bounds': extracted_bounds,
                    'original_message': message
                }
            
            successful_extractions = sum(1 for r in results.values() 
        ...
        def _test():
            ...
            for system in test_systems[:3]:  # Test first 3 systems
                ...
                results[system['name']] = {
                    ...
                    'verification_success': verification_result['overall_success'] if verification_result else False,
                    'workflow_complete': (
                        generation_result['success'] and 
                        generation_result['certificate'] is not None and
...
class WebInterfaceTestbench:
    ...
    def generate_report(self, output_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        for component in self.test_suites.keys():
            component_results = [r for r in self.results if r.component.lower().replace('testsuite', '') == component]
    ...
    def _analyze_failures_and_suggest_improvements(self, failed_results: List[TestResult]) -> List[str]:
        ...
        for component, failures in failure_patterns.items():
            ...
            if any('memory' in msg.lower() for msg in error_messages):
                suggestions.append(f"Reduce batch sizes or enable memory optimization for {component}")
            
            if any('connection' in msg.lower() for msg in error_messages):
                suggestions.append(f"Check network connectivity and service availability for {component}")
...
def main():
    ...
    parser.add_argument("--component", type=str, choices=['certificate_generator', 'verification_service', 'conversation_service', 'integration'], help="Run tests for specific component only")
    parser.add_argument("--benchmark", action="store_true", help="Run performance benchmark")
    parser.add_argument("--iterations", type=int, default=10, help="Number of benchmark iterations")
    parser.add_argument("--improve", action="store_true", help="Run quality improvement cycle")
    parser.add_argument("--max-improve-iterations", type=int, default=5, help="Maximum improvement iterations")
    ...
    try:
        if args.component:
            ...
            logger.info(f"Component {args.component}: {len([r for r in results if r.status == 'PASS'])}/{len(results)} tests passed")
            ...
            logger.info(f"  Min/Max Duration: {metrics.min_duration:.3f}s / {metrics.max_duration:.3f}s")
        
        elif args.improve:
            # Run quality improvement cycle
            improvement_results = testbench.run_quality_improvement_cycle(args.max_improve_iterations)
            logger.info(f"Quality Improvement Results:")
            ...
            total_passed = sum(len([r for r in component_results if r.status == 'PASS']) for component_results in results.values())
            total_tests = sum(len(component_results) for component_results in results.values())
            logger.info(f"Overall Results: {total_passed}/{total_tests} tests passed ({total_passed/total_tests:.1%})")
