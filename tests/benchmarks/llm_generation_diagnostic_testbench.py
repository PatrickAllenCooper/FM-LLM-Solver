...
class LLMGenerationDiagnosticTestbench:
    ...
    def _analyze_result(self, result):
        ...
        logger.info(f"📝 LLM Output Preview: {llm_output[:200]}{'...' if len(llm_output) > 200 else ''}")
    ...
    def test_improved_prompts(self):
        ...
        logger.info(f"📊 Current prompt analysis:")
        ...
        for name, prompt in improved_prompts.items():
            logger.info(f"📏 {name.capitalize()} prompt: {len(prompt)} chars ({len(current_prompt) - len(prompt):+d} vs current)")
    ...
    def _create_minimal_prompt(self, system_desc, domain_bounds):
        ...
        return f"""<s>[INST] Generate a barrier certificate:
...
B(x, y) = [/INST]"""
    ...
    def _create_direct_prompt(self, system_desc, domain_bounds):
        ...
        return f"""<s>[INST] Generate a barrier certificate for:
...
B(x, y) = [/INST]"""
    ...
    def _create_constrained_prompt(self, system_desc, domain_bounds):
        ...
        return f"""<s>[INST] Complete the barrier certificate:
