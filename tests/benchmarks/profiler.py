...
class BarrierCertificateProfiler:
    ...
    def profile_validation(self, 
                          ...
                          config: DictConfig,
                          test_name: str = "unnamed") -> ProfileResult:
        ...
        bottlenecks = [f"{name}: {time:.3f}s ({call_counts.get(name, 0)} calls)" 
        ...
        for func, time in function_times.items():
            if time > 0.5 * total_time:  # Function takes >50% of total time
                recommendations.append(
                    ...
                    f"of total execution time. Consider optimization."
                )
    ...
    def run_benchmark_suite(self):
        ...
        for test_case in test_cases:
            for num_samples in test_case['samples']:
                ...
                print(f"  Top bottleneck: {result.bottlenecks[0] if result.bottlenecks else 'None'}")
    ...
    def _generate_text_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"Min/Max time: {summary.get('min_time', 0):.3f}s / {summary.get('max_time', 0):.3f}s\n")
