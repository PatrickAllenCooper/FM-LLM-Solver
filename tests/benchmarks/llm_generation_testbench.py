...
class LLMGenerationTestbench:
    ...
    def run_single_test(self, test_case: TestCase, model_config: str, rag_k: int, 
                       attempt: int = 1) -> TestResult:
        """Run a single test case and return results."""
        logger.info(f"Running test '{test_case.name}' with model '{model_config}', RAG k={rag_k}, attempt {attempt}")
        ...
        try:
            ...
            logger.info(f"Verification complete - Overall: {result.verification_results['overall_success']}")
    ...
    def run_test_suite(self, model_configs: List[str] = None, rag_k_values: List[int] = None,
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        successful_verifications = sum(1 for r in self.results if r.verification_results.get('overall_success', False))
        ...
        analysis['summary'] = {
            ...
            'verification_success_rate': successful_verifications / total_tests if total_tests > 0 else 0
        }
    ...
    def run_focused_test(self, system_description: str, domain_bounds: Dict[str, List[float]],
                        model_config: str = "base", rag_k: int = 3) -> TestResult:
        ...
        print("-"*60)
        print("LLM Output:")
        print(result.llm_output[:500] + "..." if len(result.llm_output) > 500 else result.llm_output)
...
def main():
    ...
    if args.focused:
        ...
        testbench.run_focused_test(system, domain, args.models[0] if args.models else "base", args.rag_k[0])
    else:
        # Run full test suite
