...
def generate_summary() -> str:
    ...
    if system_status['gpu_available']:
        summary.append(f"  GPU: {system_status['gpu_name']} ({system_status['gpu_memory_gb']:.1f} GB)")
    else:
    ...
    summary.append("  • GPU acceleration is ready" if system_status['gpu_available'] else "  • CPU-only mode available")
