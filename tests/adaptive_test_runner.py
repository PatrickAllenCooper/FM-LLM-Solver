...
class AdaptiveTestRunner:
    ...
    def run_adaptive_tests(
        ...
        test_strategy = self._build_test_strategy(
            test_scope, include_categories, exclude_categories
        )
    ...
    def _build_test_strategy(
        self, scope: str, include: Optional[Set[str]], exclude: Optional[Set[str]]
    ) -> Dict:
        """Build test strategy based on scope and environment."""

        # Define test categories by scope
        scope_categories = {
            "essential": ["unit_tests", "core_integration", "basic_security"],
            "comprehensive": [
                "unit_tests",
                "integration_tests",
                "security_tests",
    ...
    def _run_test_category(
        self, category: str, parallel_jobs: int, timeout_multiplier: float
    ) -> Dict:
    ...
    def _run_performance_tests(self, timeout_multiplier: float) -> Dict:
        ...
        if Path("scripts/performance_benchmark.py").exists():
            return self._execute_script(
                "scripts/performance_benchmark.py", timeout_multiplier * 600
            )
        else:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "10",
                    "--duration",
                    f"{int(30 * timeout_multiplier)}s",
                    "tests/performance/load-test.js",
                ],
                capture_output=True,
                text=True,
                timeout=120 * timeout_multiplier,
            )
    ...
    def _run_monitoring_tests(self, timeout_multiplier: float) -> Dict:
        ...
        cmd = [
            ...
            "pytest",
            "-k",
            "monitoring or metrics or health",
            "-v",
            f"--timeout={int(180 * timeout_multiplier)}",
        ]
    ...
    def _generate_recommendations(self, success_rate: float, duration: float):
        ...
        if self.environment_type == "deployed" and success_rate >= 95:
            recommendations.append(
                "All tests passing in deployment environment - system is production ready"
            )
...
def main():
    ...
    parser.add_argument(
        "--environment",
        choices=["macbook", "desktop", "deployed"],
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show strategy without running tests"
    )
