...
BARRIER_CERTIFICATE_END""",
            ...
            f"""Based on the system dynamics, I propose the following barrier certificate:

B(x,y) = {certificate}

This certificate satisfies the barrier conditions.""",
            
            f"""Here's the barrier certificate for the given system:
...
Additional analysis shows this certificate is valid.""",
            ...
            f"""The optimal barrier certificate is:
...
This provides the required safety guarantees."""
    ...
    def test_certificate_extraction(self, test_case: Dict) -> Dict:
        ...
        return {
            ...
            "results": results,
            "success_rate": sum(1 for r in results if r['extraction_success'] and not r['template_rejected']) / len(results)
        }
    ...
    def save_test_results(self, results: Dict, output_path: str = "test_results/certificate_pipeline_results.json"):
...
def test_certificate_extraction_pipeline(pipeline_tester):
    ...
    for test_case in test_cases[:3]:  # Test first 3 cases
        ...
        assert result["success_rate"] > 0.5, f"Extraction success rate too low for {test_case['name']}"
...
def test_comprehensive_pipeline(pipeline_tester):
    ...
    assert results["overall_success_rate"] > 0.6, f"Overall success rate too low: {results['overall_success_rate']}"
