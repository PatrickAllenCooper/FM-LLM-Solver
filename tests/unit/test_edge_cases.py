...
def test_edge_cases():
    ...
    test_cases = [
        ...
        ("B(x,y) = 2*x**2 + 3*y**2 - 1.5", ["x", "y"], "2*x**2 + 3*y**2 - 1.5", "Weighted quadratic"),
        ("B(x,y,z) = x**2 + y**2 + z**2 - 2.0", ["x", "y", "z"], "x**2 + y**2 + z**2 - 2.0", "3D certificate"),
        
        # Multiple certificates (should extract first valid one)
        ("B(x,y) = x**2 + y**2 - 1\nB(x,y) = x**2 + y**2 - 2", ["x", "y"], "x**2 + y**2 - 1", "Multiple certificates"),
        
        # Nested expressions
        ("The certificate is B(x,y) = (x**2 + y**2) - 1.0", ["x", "y"], "x**2 + y**2 - 1.0", "Parentheses"),
