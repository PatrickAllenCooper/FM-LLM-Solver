...
def test_stochastic_classification():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(f"  Stochastic: {is_stochastic} (expected: {test_case['expected_stochastic']}) {'✅' if stochastic_correct else '❌'}")
        print(f"  Confidence: {confidence:.3f}")
        print(f"  Keywords found: {details.get('stochastic_count', 0)}")
        print(f"  Matches: {details.get('stochastic_matches', [])[:3]}")
        print(f"  Include: {should_include} (expected: {test_case['expected_include']}) {'✅' if include_correct else '❌'}")
...
def test_keyword_threshold():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(f"Test {i}: {details.get('stochastic_count', 0)} keywords → Stochastic: {is_stochastic} {'✅' if correct else '❌'}")
