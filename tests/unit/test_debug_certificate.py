...
print("   At boundary (r=0.5): B =", float(B.subs({'x': 0.5, 'y': 0})), "✓" if float(B.subs({'x': 0.5, 'y': 0})) <= 0 else "✗")

print("2. Unsafe set (x²+y² ≥ 4.0): B should be > 0")  
print("   At boundary (r=2.0): B =", float(B.subs({'x': 2.0, 'y': 0})), "✓" if float(B.subs({'x': 2.0, 'y': 0})) > 0 else "✗")
