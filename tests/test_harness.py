...
class BarrierCertificateTestHarness:
    ...
    def run_all_tests(self, subset: Optional[List[str]] = None):
        ...
        for i, test_case in enumerate(test_list):
            ...
            if result.correct:
                self.logger.info("✓ PASS: Result matches expectation")
            else:
                self.logger.warning("✗ FAIL: Result does not match expectation")
    ...
    def run_single_test(self, test_case: Dict[str, Any]) -> TestResult:
        ...
        try:
            ...
            new_result = self._run_new_validator(test_case["certificate"], system_info)
            ...
            if result.level_sets_expected and result.level_sets_computed:
                c1_match = (
                    abs(
                        result.level_sets_computed.get("c1", 0)
                        - result.level_sets_expected.get("c1", 0)
                    )
                    < 0.1
                )
                c2_match = (
                    abs(
                        result.level_sets_computed.get("c2", 0)
                        - result.level_sets_expected.get("c2", 0)
                    )
                    < 0.1
                )
        ...
        try:
            start_time = time.time()
            old_result = self._run_old_validator(test_case["certificate"], system_info)
        ...
        # Analyze results
        result.agreement = (
            result.new_validator_result == result.old_validator_result
    ...
    def _run_new_validator(self, certificate: str, system_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        validator = BarrierCertificateValidator(
            certificate_str=certificate, system_info=system_info, config=config
        )
    ...
    def generate_report(self, output_file: str = "test_harness_report.txt"):
        ...
        with open(output_file, "w") as f:
            ...
            if agreements:
                f.write(
                    f"Validator Agreement: {agree_count}/{len(agreements)} ({100*agree_count/len(agreements):.1f}%)\n\n"
                )
            ...
            for result in self.results:
                ...
                if result.level_sets_computed:
                    f.write("\nLevel Sets:\n")
                    f.write(
                        f"  Computed: c1={result.level_sets_computed.get('c1', 'N/A'):.3f}, "
                        f"c2={result.level_sets_computed.get('c2', 'N/A'):.3f}\n"
                    )
                    if result.level_sets_expected:
                        f.write(
                            f"  Expected: c1={result.level_sets_expected.get('c1', 'N/A'):.3f}, "
            ...
            if failed_tests:
                ...
                for result in failed_tests:
                    f.write(
                        f"{result.test_id}: Expected {result.expected_valid}, "
