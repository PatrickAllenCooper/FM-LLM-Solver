...
class TestUserAccountSystem:
    ...
    def test_user_activity_tracking(self, client, sample_user):
        ...
        with app.app_context():
            ...
            activities = UserActivity.query.filter_by(
                user_id=sample_user.id, activity_type="login"
            ).all()
    ...
    def test_api_key_management(self, client, sample_user):
        ...
        # Generate API key
        response = client.post(
            "/auth/api_key", headers={"X-Requested-With": "XMLHttpRequest"}
        )
        assert response.status_code == 200
        ...
        response = client.post(
            "/auth/revoke_api_key", headers={"X-Requested-With": "XMLHttpRequest"}
        )
        assert response.status_code == 200
...
class TestUserAccountIntegration:
    ...
    def test_certificate_generation_with_user_tracking(self, client):
        ...
        client.post("/auth/login", data={"username": "certuser", "password": "pass123"})

        # Mock certificate generation
        with patch(
            "web_interface.certificate_generator.CertificateGenerator"
        ) as mock_gen:
            ...
            with app.app_context():
                ...
                activities = UserActivity.query.filter_by(
                    user_id=user.id, activity_type="certificate_generated"
                ).all()
