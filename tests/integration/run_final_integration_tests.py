...
def main():
    ...
    try:
        ...
        print(f"   {'✅' if config_success else '❌'} Configuration loading: {'PASS' if config_success else 'FAIL'}")
...
Unsafe Set: x >= 1.5"""
        ...
        print(f"   {'✅' if parsing_success else '❌'} System parsing: {'PASS' if parsing_success else 'FAIL'}")
...
BARRIER_CERTIFICATE_END"""
                ...
                specific_success = (specific_extracted is not None and 
                                  not cert_gen._is_template_expression(specific_extracted))
...
BARRIER_CERTIFICATE_END"""
                ...
                template_correctly_rejected = (template_extracted is None or 
...
BARRIER_CERTIFICATE_END"""
                ...
                simple_correctly_rejected = (simple_extracted is None or 
                                           cert_gen._is_template_expression(simple_extracted))
                
                text_processing_success = (specific_success and 
                                         template_correctly_rejected and 
                                         simple_correctly_rejected)
                
                test_results['text_processing'] = text_processing_success
                print(f"   {'✅' if specific_success else '❌'} Specific certificate extraction: {'PASS' if specific_success else 'FAIL'}")
                print(f"   {'✅' if template_correctly_rejected else '❌'} Template rejection: {'PASS' if template_correctly_rejected else 'FAIL'}")
                print(f"   {'✅' if simple_correctly_rejected else '❌'} Generic expression rejection: {'PASS' if simple_correctly_rejected else 'FAIL'}")
        ...
        try:
            ...
            print(f"   {'✅' if verification_success else '❌'} Complete workflow: {'PASS' if verification_success else 'FAIL'}")
        ...
        if success_rate >= 0.85:
            ...
            failing_components = [name for name, key in components if not test_results.get(key, False)]
