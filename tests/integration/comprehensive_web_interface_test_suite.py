...
Unsafe Set: x**4 + y**4 >= 1.0"""
    ...
    def test_certificate_generation(self, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            print(f"   {status} - Overall: {overall_success}, Numerical: {numerical_passed}, SOS: {sos_passed}")
            ...
            try:
                if "system" in test_config:
                    ...
                    if result.get('overall_success') == False:
                        ...
                        print(f"   ✅ Graceful failure as expected")
                    else:
                        print(f"   ⚠️ Unexpected success")
                        
                else:
                    # Test system parsing with invalid input
                    parsed = self.verification_service.parse_system_description(test_config["input"])
                    
                    if not parsed.get('variables') or not parsed.get('dynamics'):
                        error_results["graceful_failures"] += 1
                        print(f"   ✅ Graceful parsing failure as expected")
                    else:
                        print(f"   ⚠️ Unexpected parsing success")
...
Unsafe Set: x**2 + y**2 >= 4.0"""
        ...
        for samples in sample_counts:
            ...
            try:
                ...
                print(f"   ⏱️ {elapsed:.2f}s ({samples/elapsed:.0f} samples/sec) - {'✅' if success else '❌'}")
    ...
    def _generate_comprehensive_report(self, verification_results: Dict, error_results: Dict, 
                                     performance_results: Dict, llm_available: bool, 
                                     suite_time: float) -> Dict[str, Any]:
        ...
        if self.test_results:
            avg_generation_time = np.mean([r.generation_time for r in self.test_results if r.generation_time])
            avg_verification_time = np.mean([r.verification_time for r in self.test_results if r.verification_time])
        else:
        ...
        # Component health
        component_health = {
            'verification_service': verification_results['tests_passed'] / verification_results['tests_run'] if verification_results['tests_run'] > 0 else 0,
            'boundary_fix': verification_results.get('boundary_fix_working', False),
            'error_handling': error_results['graceful_failures'] / error_results['tests_run'] if error_results['tests_run'] > 0 else 0,
            'llm_components': llm_available,
            'performance': 'good' if performance_results.get('scalability') == 'good' else 'needs_attention'
        }
        
        # Overall assessment
        critical_components_working = (
            component_health['verification_service'] >= 0.8 and
            component_health['boundary_fix'] and
            component_health['error_handling'] >= 0.8
        )
    ...
    def _generate_recommendations(self, component_health: Dict, success_rate: float) -> List[str]:
        ...
        if not component_health.get('boundary_fix'):
            recommendations.append("CRITICAL: Boundary condition fix not working - investigate verification system")
        
        if component_health.get('verification_service', 0) < 0.8:
            recommendations.append("Verification service needs attention - check parsing and validation logic")
        ...
        if not component_health.get('llm_components'):
            recommendations.append("LLM components not available - consider mock testing or model loading fixes")
        
        if success_rate < 0.7:
            recommendations.append("Success rate below target - review certificate generation and verification")
