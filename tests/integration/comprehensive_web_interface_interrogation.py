    ...
    def test_verification_consistency(self) -> List[VerificationConsistencyResult]:
        ...
        for cert_info in known_certificates:
            ...
            try:
                ...
                results = [r for r in [sos_result, numerical_result, symbolic_result] if r is not None]
                ...
                if len(results) > 1:
                    ...
                    if expected_result is not None:
                        theory_agreement = all(r == expected_result for r in results if r is not None)
                        if theory_agreement:
                ...
                if sos_result is not None and numerical_result is not None:
                    if sos_result != numerical_result:
                        discrepancies.append(f"SOS ({sos_result}) vs Numerical ({numerical_result})")
                
                if expected_result is not None:
                    if overall_result != expected_result:
                        discrepancies.append(f"Overall ({overall_result}) vs Expected ({expected_result})")
                
                consistency_results.append(VerificationConsistencyResult(
                ...
                status = "✅" if consistency_score >= 0.8 else "⚠️" if consistency_score >= 0.5 else "❌"
...
Domain: x ∈ [-3, 3], y ∈ [-3, 3]""",
        ...
        for system_info in test_systems:
            try:
                ...
                domain_bounds_included = system_info["domain_bounds"] is not None and "Domain" in prompt
                
                analyses.append(LLMPromptAnalysis(
                ...
                logger.info(f"   📝 {system_info['name']}: {len(qwen_optimizations)} Qwen optimizations detected")
...
BARRIER_CERTIFICATE_END""",
        ...
        for test in test_outputs:
            try:
                # Test extraction
                extracted = self.certificate_generator.extract_certificate_from_output(test["output"])
                parsing_successful = extracted is not None
                
                # Test template detection
                template_detected = False
                if extracted:
                    template_detected = self.certificate_generator._is_template_expression(extracted)
                ...
                parsing_results.append(ParsingAccuracyResult(
                    raw_output=test["output"][:100] + "..." if len(test["output"]) > 100 else test["output"],
                ...
                status = "✅" if confidence_score >= 0.8 else "⚠️" if confidence_score >= 0.5 else "❌"
...
Unsafe Set: x**2 + y**2 >= 4.0"""
        ...
        try:
            ...
            parsing_consistent = (
                parsed_system1.get('variables') == parsed_system2.get('variables') and
                parsed_system1.get('dynamics') == parsed_system2.get('dynamics')
            )
            ...
            verification_consistent = (
                verification1.get('overall_success') == verification2.get('overall_success') and
                verification1.get('sos_passed') == verification2.get('sos_passed')
            )
    ...
    def _generate_interrogation_report(self, total_time: float) -> Dict[str, Any]:
        ...
        if llm_analysis:
            ...
            report['summary']['llm_prompting'] = {
                ...
                'domain_bounds_integration': sum(1 for a in llm_analysis if a.domain_bounds_included)
            }
        ...
        if cross_component:
            ...
            report['summary']['cross_component_consistency'] = {
                ...
                'consistency_rate': consistent_components / total_components if total_components > 0 else 0
            }
...
def main():
    ...
    try:
        ...
        if 'llm_prompting' in summary:
            ...
            print(f"   Domain bounds integration: {lp['domain_bounds_integration']}/{lp['prompt_analyses_run']} prompts")
        ...
        if 'cross_component_consistency' in summary:
            ...
            print(f"   Consistent components: {cc['consistent_components']}/{cc['total_components']}")
