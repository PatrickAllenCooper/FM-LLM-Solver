    ...
    def setup_components(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with patch('web_interface.conversation_service.db') as mock_db:
                ...
                setup_results['conversation'] = {'success': True, 'initialized': True, 'mocked_db': True}
...
Domain: x ∈ [-3, 3], y ∈ [-2, 2]"""
        ...
        return {
            ...
            'suggestions': [] if successful_parsing >= 2 else [
...
BARRIER_CERTIFICATE_END"""
        ...
        return {
            ...
            'template_correctly_rejected': not results['template_rejection']['extraction_successful'],
            'detailed_results': results,
            'suggestions': [] if successful_extractions >= 2 else [
    ...
    def _test_domain_bounds_handling(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'suggestions': [] if (verification_working and cert_gen_working) else [
...
Unsafe Set: x**2 + y**2 >= 2.0"""
        ...
        try:
            ...
            result = verification_service.verify_certificate(
                test_certificate,
                test_system,
                param_overrides={'num_samples_lie': 1000, 'num_samples_boundary': 500}  # Reduced for speed
            )
...
Unsafe Set: x + y >= 2.0"""
        ...
        for test_case in test_cases:
            try:
                ...
                cleaned_cert = verification_service._clean_certificate_string(test_case['certificate'])
                ...
                workflow_results[test_case['name']] = {
                    ...
                    'bounds_created': len(bounds) > 0,
                    'certificate_cleaned': cleaned_cert == test_case['certificate'],  # Should be unchanged for these
        ...
        return {
            ...
            ] if successful_workflows > 0 else [
    ...
    def _test_conversation_extraction_workflow(self) -> Dict[str, Any]:
        ...
        return {
            ...
            ] if successful_extractions >= 2 else [
    ...
    def _test_error_handling_robustness(self) -> Dict[str, Any]:
        ...
        if 'verification' in self.components:
            try:
                ...
                parsed = verification_service.parse_system_description("This is not a system description")
                error_tests.append({
                    'test': 'malformed_system_parsing',
                    'handled_gracefully': isinstance(parsed, dict),  # Should return empty dict, not crash
                    'result': parsed
                })
                ...
                error_tests.append({
                    ...
                    'handled_gracefully': isinstance(result, dict),  # Should return result dict, not crash
                    'success': result.get('overall_success', False) if isinstance(result, dict) else False
                })
        ...
        if 'cert_gen' in self.components:
            try:
                ...
                extracted = cert_gen.extract_certificate_from_output("Random text with no certificate")
                error_tests.append({
        ...
        return {
            'success': gracefully_handled >= len(error_tests) * 0.8,  # 80% should handle errors gracefully
            ...
            ] if gracefully_handled >= len(error_tests) * 0.8 else [
...
BARRIER_CERTIFICATE_END"""
    ...
    def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        categories = {
            ...
            'robustness': [r for r in self.results if 'error_handling' in r.test_name or 'performance' in r.test_name]
        }
    ...
    def _generate_next_steps(self, readiness_level: str, category_stats: Dict) -> List[str]:
        ...
        for cat_name, stats in category_stats.items():
            if stats['success_rate'] < 0.8:
                steps.append(f"Improve {cat_name} functionality (currently {stats['success_rate']:.1%} success rate)")
...
def main():
    ...
    try:
        ...
        for cat_name, stats in report['category_breakdown'].items():
            print(f"  {cat_name.title()}: {stats['passed']}/{stats['total']} ({stats['success_rate']:.1%})")
