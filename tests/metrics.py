...
class MetricsCalculator:
    ...
    def generate_classification_report(self) -> str:
        ...
        report.append(f"Actual Invalid     {self.metrics.true_negatives:4d}   {self.metrics.false_positives:4d}")
        report.append(f"       Valid       {self.metrics.false_negatives:4d}   {self.metrics.true_positives:4d}")
        report.append("")
    ...
    def analyze_failure_patterns(self) -> Dict[str, Any]:
        ...
        if failures['false_negatives']:
            failures['patterns']['false_negative_rate'] = len(failures['false_negatives']) / len(self.results)
            
        if failures['false_positives']:
            failures['patterns']['false_positive_rate'] = len(failures['false_positives']) / len(self.results)
...
def main():
    ...
    if args.compare:
        ...
        labels = args.labels if args.labels else [f"Validator {i+1}" for i in range(len(args.compare))]
