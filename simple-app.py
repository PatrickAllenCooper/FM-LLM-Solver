from flask import Flask, jsonify
import os

app = Flask(__name__)

@app.route('/')
def hello():
    return '''
    <html>
    <head><title>FM-LLM Solver - Hybrid Mode</title></head>
    <body style="font-family: Arial, sans-serif; margin: 40px;">
        <h1 style="color: #2196F3;">🚀 FM-LLM Solver - Hybrid Mode</h1>
        <div style="background: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <h2>✅ Web Interface Running!</h2>
            <p><strong>Status:</strong> Ready for GPU provider integration</p>
            <p><strong>Mode:</strong> Hybrid Cloud Deployment</p>
            <p><strong>External IP:</strong> Connected via GKE LoadBalancer</p>
        </div>
        <div style="background: #e8f5e8; padding: 15px; border-radius: 8px;">
            <h3>Next Steps:</h3>
            <ul>
                <li>✅ Web interface deployed on GCP</li>
                <li>🔄 GPU inference integration (RunPod/Modal)</li>
                <li>🔄 Connect web frontend to GPU backend</li>
            </ul>
        </div>
        <p><a href="/health">Health Check</a></p>
    </body>
    </html>
    '''

@app.route('/health')
def health():
    return jsonify({
        "status": "healthy",
        "mode": "hybrid",
        "service": "fm-llm-web-hybrid",
        "ready": True
    })

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, debug=False) 