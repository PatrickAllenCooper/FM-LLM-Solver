...
def format_prompt_with_context(system_description, context, kb_type="unified"):
    ...
    if is_discrete_time or kb_type == "discrete":
        type_guidance = (
            ...
            f"- For this specific system type, good concrete barriers often include:\n"
            f"  • Linear separating barriers: B(x,y) = x - 1.5 (for unsafe regions like x ≥ 1.5)\n"
            f"  • Quadratic level sets: B(x,y) = x**2 + y**2 - 2.0 (for circular regions)\n"
            f"  • Mixed forms: B(x,y) = x**2 + 0.5*y - 1.2 (combining quadratic and linear terms)\n"
            f"- Avoid placeholder templates - provide actual numerical coefficients\n\n"
        )
        discrete_conditions = (
    ...
    instruction = (
        ...
        f"BARRIER_CERTIFICATE_END\n\n"
        f"For discrete-time systems, good concrete examples include:\n"
        f"• Linear barriers: B(x, y) = x - 1.5\n"
        f"• Level set barriers: B(x, y) = x**2 + y**2 - 2.0\n"
        f"• Mixed barriers: B(x, y) = x**2 + 0.5*y - 1.0\n\n"
