...
class CertificateVerifier(Verifier):
        ...
        try:
            ...
            for verification_method in methods_to_run:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Verification failed for method {verification_method.value}: {e}"
                    )
                    checks.append(
                        VerificationCheck(
            ...
            self.logger.info(
                f"Verification completed in {computation_time:.2f}s. Valid: {overall_valid}"
            )
            return result
    ...
    def _verify_numerical(
        self, system: SystemDescription, certificate: BarrierCertificate, **kwargs
    ) -> VerificationCheck:
