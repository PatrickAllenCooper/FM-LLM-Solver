...
from flask import (
    ...
    current_app, g, redirect, url_for, flash
)
from flask_login import login_required, current_user

from fm_llm_solver.core.logging import get_logger
from fm_llm_solver.web.models import db, QueryLog, VerificationResult, Conversation, ConversationMessage
...
def index():
    ...
    try:
        ...
        if model_configs and hasattr(model_configs, 'get_available_models'):
            ...
            model_configs = [
                {'key': 'base', 'name': 'Base Model', 'description': 'Standard barrier certificate generation'},
                {'key': 'finetuned', 'name': 'Fine-tuned Model', 'description': 'Optimized for barrier certificates'},
            ]
...
def query_history():
    ...
    try:
        ...
        queries = db.session.query(QueryLog).filter_by(
            user_id=current_user.id
        ).order_by(QueryLog.timestamp.desc()).paginate(
...
def _serialize_query(query: QueryLog) -> Dict[str, Any]:
    ...
    verification_result = db.session.query(VerificationResult).filter_by(
        query_id=query.id
    ).first()
    ...
    return {
        ...
        'verification': {
            'overall_success': verification_result.overall_success if verification_result else False,
            'numerical_passed': verification_result.numerical_check_passed if verification_result else False,
            'symbolic_passed': verification_result.symbolic_check_passed if verification_result else False,
            'sos_passed': verification_result.sos_check_passed if verification_result else False,
