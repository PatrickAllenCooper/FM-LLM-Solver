...
def setup_error_handlers(app: Flask) -> None:
    ...
    def handle_validation_error(e: ValidationError):
        ...
        logger.warning(f"Validation error: {e}", extra={'request_id': getattr(g, 'request_id', None)})
        return jsonify(e.to_dict()), 400
    
    @app.errorhandler(AuthenticationError)
    def handle_authentication_error(e: AuthenticationError):
        """Handle authentication errors."""
        logger.warning(f"Authentication error: {e}", extra={'request_id': getattr(g, 'request_id', None)})
        return jsonify(e.to_dict()), 401
    
    @app.errorhandler(AuthorizationError)
    def handle_authorization_error(e: AuthorizationError):
        """Handle authorization errors."""
        logger.warning(f"Authorization error: {e}", extra={'request_id': getattr(g, 'request_id', None)})
        return jsonify(e.to_dict()), 403
    
    @app.errorhandler(RateLimitError)
    def handle_rate_limit_error(e: RateLimitError):
        """Handle rate limit errors."""
        logger.warning(f"Rate limit error: {e}", extra={'request_id': getattr(g, 'request_id', None)})
        return jsonify(e.to_dict()), 429
    
    @app.errorhandler(FMLLMSolverError)
    def handle_application_error(e: FMLLMSolverError):
        """Handle general application errors."""
        logger.error(f"Application error: {e}", extra={'request_id': getattr(g, 'request_id', None)})
