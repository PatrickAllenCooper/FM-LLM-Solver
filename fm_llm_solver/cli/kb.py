...
@click.pass_context
def build(ctx, pdf_dir: Optional[str], output_dir: Optional[str], force: bool, 
          batch_size: int, use_mathpix: bool, cpu_only: bool):
...
def info(ctx, kb_path: Optional[str]):
    ...
    if metadata_files:
        ...
        try:
            ...
            click.echo(f"  • Sources: {len(set(chunk.get('source', '') for chunk in metadata.get('chunks', [])))}")
