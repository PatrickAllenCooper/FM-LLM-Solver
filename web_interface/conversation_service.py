...
from web_interface.models import Conversation, ConversationMessage, db
...
class ConversationService:
    ...
    def send_message(
        ...
        try:
            # Find conversation
            conversation = (
                db.session.query(Conversation).filter_by(session_id=session_id).first()
            )
            if not conversation:
            ...
            extracted_description = self._extract_system_description_from_conversation(
                conversation
            )
            ...
            extracted_bounds = self._extract_domain_bounds_from_conversation(
                conversation
            )
            if extracted_bounds:
    ...
    def set_ready_to_generate(self, session_id: str, ready: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "ready_to_generate": ready}
    ...
    def generate_certificate_from_conversation(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Update conversation status
            conversation.status = (
                "active"  # Return to active for potential further conversation
            )
            db.session.commit()

            return {"success": result["success"], "certificate_result": result}
    ...
    def _generate_conversation_response(
        ...
        try:
            # Load model
            model_info = self.certificate_generator._load_model(
                conversation.model_config
            )
            ...
            if conversation.rag_k > 0:
                try:
                    ...
                    kb_info = self.certificate_generator._load_knowledge_base(
                        barrier_type
                    )
                    if kb_info:
                        embedding_model = (
                            self.certificate_generator._load_embedding_model()
                        )
                        ...
                        context_chunks = (
                            rag_context.count("--- Context Chunk") if rag_context else 0
                        )
                except Exception as e:
            ...
            if output_start_index != -1:
                response = generated_text[
                    output_start_index + len(prompt_end_marker) :
                ].strip()
            else:
            ...
            return (
                "I apologize, but I encountered an error while processing your message. Could you please try again?",
                0,
            )

    def _build_conversation_context(
        self, conversation: Conversation, current_user_message: str
    ) -> List[Dict[str, str]]:
    ...
    def _clean_conversation_response(self, response: str) -> str:
        ...
        response = (
            response.replace("[/INST]", "").replace("<s>", "").replace("</s>", "")
        )

        return response

    def _extract_system_description_from_conversation(
                ...
                if "dynamic" in content or "dx/dt" in content or "x_next" in content:
                    ...
                    for line in lines:
                        if any(
                            ...
                            for keyword in ["dynamic", "dx/dt", "x_next", "x(t+1)"]
                        ):
    ...
    def _extract_domain_bounds_from_conversation(
        self, conversation: Conversation
    ) -> Optional[dict]:
