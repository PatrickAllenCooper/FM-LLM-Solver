...
from web_interface.models import SecurityLog, User, UserActivity, UserSession, db
...
def logout():
    ...
    log_security_event(
        "logout", severity="low", description=f"User logged out: {username}"
    )
        ...
        return jsonify(
            {
                ...
                "created": (
                    current_user.api_key_created.isoformat()
                    if current_user.api_key_created
                    else None
                ),
            }
        )
