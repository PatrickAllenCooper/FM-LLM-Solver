...
class VerificationService:
    ...
    def parse_system_description(self, system_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            vars_match = re.search(
                r"State Variables:\s*\[?([^\]]+)\]?", system_description, re.IGNORECASE
            )
            if vars_match:
    ...
    def _infer_variables_from_dynamics(self, dynamics: List[str]) -> List[str]:
        ...
        for expr in dynamics:
            ...
            for var in var_matches:
                ...
                if (
                    var
                    not in ["sin", "cos", "tan", "exp", "log", "sqrt", "abs", "pi", "e"]
                    and len(var) <= 3
                ):
                    variables.add(var)
    ...
    def create_sampling_bounds(
        self, system_info: Dict[str, Any], default_range: float = 2.0
    ) -> Dict[str, tuple]:
    ...
    def _update_bounds_from_condition(
        self, bounds: Dict[str, tuple], condition: str, variables: List[str]
    ):
    ...
    def _clean_certificate_string(self, certificate_str: str) -> str:
        ...
        if cleaned != certificate_str:
            logger.debug(
                f"Cleaned certificate for verification: '{certificate_str}' -> '{cleaned}'"
            )
    ...
    def verify_certificate(
        ...
        try:
            ...
            if is_discrete:
                ...
                logger.info(
                    f"Optimized sampling bounds for discrete system: {sampling_bounds}"
                )
                ...
                if not param_overrides or "numerical_tolerance" not in param_overrides:
                    verification_cfg_dict["numerical_tolerance"] = (
                        1e-6  # More strict tolerance
                    )
                    logger.info(f"Set discrete numerical tolerance: 1e-6")
                if not param_overrides or "num_samples_lie" not in param_overrides:
                    verification_cfg_dict["num_samples_lie"] = min(
                        15000, verification_cfg_dict.get("num_samples_lie", 9100) * 1.5
                    )
                    logger.info(
                        f"Set discrete lie samples: {verification_cfg_dict['num_samples_lie']}"
                    )
                if not param_overrides or "num_samples_boundary" not in param_overrides:
                    verification_cfg_dict["num_samples_boundary"] = min(
                        8000,
                        verification_cfg_dict.get("num_samples_boundary", 4600) * 1.5,
                    )
                    logger.info(
                        f"Set discrete boundary samples: {verification_cfg_dict['num_samples_boundary']}"
                    )
            else:
                logger.info(
                    "System detected as continuous-time - using standard parameters"
                )
            ...
            try:
                ...
                if validation_result["is_valid"]:
                    logger.info(
                        "New validator passed - running traditional verification for comparison"
                    )
                    ...
                    logger.info(
                        "New validator failed - skipping traditional verification"
                    )
                    traditional_result = None

            except Exception as e:
                logger.warning(
                    f"New validator failed with error: {e}. Falling back to traditional verification."
                )
            ...
            if validation_result:
                ...
                verification_result = {
                    ...
                    "details": {
                        ...
                        "numerical": {
                            ...
                            "separation_valid": validation_result.get(
                                "separation_valid", False
                            ),
                            "lie_derivative_valid": validation_result.get(
                                "lie_derivative_valid", False
                            ),
                        },
                        ...
                        "traditional_result": (
                            traditional_result if traditional_result else None
                        ),
                    },
                }
            else:
                ...
                if isinstance(result, dict):
                    verification_result = {
                        ...
                        "numerical_passed": result.get(
                            "numerical_verification", {}
                        ).get("success", False),
                        "symbolic_passed": result.get("symbolic_verification", {}).get(
                            "success", False
                        ),
                        "sos_passed": result.get("sos_verification", {}).get(
                            "success", False
                        ),
                        "verification_time": verification_time,
                        "details": {
    ...
    def get_detailed_feedback(
        ...
        if numerical:
            status = (
                "✅ Passed" if verification_result["numerical_passed"] else "❌ Failed"
            )
        ...
        if symbolic:
            status = (
                "✅ Passed" if verification_result["symbolic_passed"] else "❌ Failed"
            )
    ...
    def _optimize_bounds_for_system(
        self, bounds: Dict[str, tuple], system_info: Dict[str, Any]
    ):
        ...
        for condition in initial_conditions:
            if "**2" in condition and "+" in condition and "<=" in condition:
                ...
                circle_match = re.search(
                    r"([a-zA-Z_]\w*)\*\*2\s*\+\s*([a-zA-Z_]\w*)\*\*2\s*<=?\s*([+-]?\d*\.?\d+)",
                    condition,
                )
                if circle_match:
                    ...
                    logger.info(
                        f"Optimized bounds for circular initial set: radius={radius:.2f}, bounds=±{margin:.2f}"
                    )

        # For unsafe boundaries like x >= value
        for condition in unsafe_conditions:
            bound_match = re.search(
                r"([a-zA-Z_]\w*)\s*>=?\s*([+-]?\d*\.?\d+)", condition
            )
            if bound_match:
                ...
                if var in bounds:
                    ...
                    new_max = min(
                        current_max, boundary_val - 0.1
                    )  # Stay safely away from boundary
                    bounds[var] = (new_min, new_max)
                    logger.info(
                        f"Optimized bounds for unsafe boundary {var} >= {boundary_val}: [{new_min:.2f}, {new_max:.2f}]"
                    )
    ...
    def _generate_safe_set_conditions(self, system_info: Dict[str, Any]) -> List[str]:
        ...
        for unsafe_condition in unsafe_conditions:
            ...
            if safe_condition:
                ...
                logger.info(
                    f"Generated safe set condition from unsafe set: {safe_condition}"
                )
        ...
        if not safe_conditions and initial_conditions:
            for init_condition in initial_conditions:
                ...
                if expanded_condition:
                    ...
                    logger.info(
                        f"Generated safe set condition from initial set: {expanded_condition}"
                    )
    ...
    def _expand_condition(self, condition: str) -> str:
        ...
        if circle_match:
            ...
            expanded_radius_sq = (
                float(radius_sq) * 4
            )  # Expand by factor of 2 in radius (4 in area)
