...
class MonitoringService:
    ...
    def get_usage_metrics(self, time_range: str = "all") -> UsageMetrics:
        ...
        metrics.total_requests = (
            db.session.query(QueryLog).filter(*query_filter).count()
        )
            ...
            .filter(
                *query_filter,
                or_(
                    ...
                    QueryLog.generated_certificate.is_(None),
                ),
            )
            .count()
        )
        ...
        metrics.active_users_today = (
            db.session.query(func.count(func.distinct(QueryLog.user_id)))
            .filter(func.date(QueryLog.timestamp) == today)
            .scalar()
            or 0
        )

        week_ago = datetime.utcnow() - timedelta(days=7)
        metrics.active_users_week = (
            db.session.query(func.count(func.distinct(QueryLog.user_id)))
            .filter(QueryLog.timestamp >= week_ago)
            .scalar()
            or 0
        )

        month_ago = datetime.utcnow() - timedelta(days=30)
        metrics.active_users_month = (
            db.session.query(func.count(func.distinct(QueryLog.user_id)))
            .filter(QueryLog.timestamp >= month_ago)
            .scalar()
            or 0
        )
        ...
        if completed_queries:
            total_gen_time = sum(
                (q.processing_end - q.processing_start).total_seconds()
        ...
        # Verification times
        verifications = (
            db.session.query(VerificationResult)
            .filter(VerificationResult.verification_time_seconds.isnot(None))
            .all()
        )

        if verifications:
            metrics.avg_verification_time = sum(
                v.verification_time_seconds for v in verifications
            ) / len(verifications)
        ...
        metrics.total_web_calls = (
            db.session.query(QueryLog)
            .filter(*query_filter, QueryLog.conversation_id.isnot(None))
            .count()
        )
    ...
    def get_cost_metrics(self, time_range: str = "month") -> CostMetrics:
        ...
        metrics.api_calls = (
            ...
            .filter(
                QueryLog.timestamp >= start_date,
                QueryLog.conversation_id.is_(
                    None
                ),  # Assuming API calls don't have conversation
            )
            .count()
        )
        metrics.api_cost = (metrics.api_calls / 1000) * self.cost_config[
            "api_cost_per_1k"
        ]
        ...
        metrics.storage_cost = (
            metrics.storage_gb
            * self.cost_config["storage_cost_per_gb_month"]
            * days_in_period
            / 30
        )
        ...
        total_queries = (
            db.session.query(QueryLog).filter(QueryLog.timestamp >= start_date).count()
        )
        metrics.bandwidth_gb = (total_queries * 1) / 1024  # 1MB per query in GB
        metrics.bandwidth_cost = (
            metrics.bandwidth_gb * self.cost_config["bandwidth_cost_per_gb"]
        )
    ...
    def get_system_metrics(self) -> SystemMetrics:
        ...
        total_recent = (
            db.session.query(QueryLog).filter(QueryLog.timestamp >= hour_ago).count()
            or 1
        )

        failed_recent = (
            db.session.query(QueryLog)
            .filter(QueryLog.timestamp >= hour_ago, QueryLog.status == "failed")
            .count()
        )
    ...
    def get_certificate_history(
        ...
        for q in queries:
            ...
            verification = (
                db.session.query(VerificationResult).filter_by(query_id=q.id).first()
            )

            # Get user info
            user = (
                db.session.query(User).filter_by(id=q.user_id).first()
                if q.user_id
                else None
            )
            ...
            history.append(
                {
                    ...
                    "user_id": q.user_id,
                    "system_description": (
                        q.system_description[:100] + "..."
                        if len(q.system_description) > 100
                        else q.system_description
                    ),
                    ...
                    "verification": (
                        {
                            "overall_success": (
                                verification.overall_success if verification else None
                            ),
                            "numerical": (
                                verification.numerical_check_passed
                                if verification
                                else None
                            ),
                            "symbolic": (
                                verification.symbolic_check_passed
                                if verification
                                else None
                            ),
                            "sos": (
                                verification.sos_check_passed if verification else None
                            ),
                        }
                        if verification
                        else None
    ...
    def get_user_statistics(self, user_id: int) -> Dict[str, Any]:
        ...
        queries_today = (
            db.session.query(QueryLog)
            .filter(QueryLog.user_id == user_id, func.date(QueryLog.timestamp) == today)
            .count()
        )

        # Rate limit history
        rate_limit_violations = (
            db.session.query(RateLimitLog)
            .filter_by(user_id=user_id, was_blocked=True)
            .count()
        )
        ...
        model_usage = (
            db.session.query(QueryLog.model_config, func.count(QueryLog.id))
            .filter_by(user_id=user_id)
            .group_by(QueryLog.model_config)
            .all()
        )
        ...
        return {
            ...
            "statistics": {
                ...
                "queries_remaining_today": user.daily_request_limit
    ...
    def get_trending_systems(
        ...
        trending = (
            db.session.query(
                QueryLog.system_description, func.count(QueryLog.id).label("count")
            )
            .filter(QueryLog.timestamp >= since)
            .group_by(QueryLog.system_description)
        ...
        return [
            {"system": t[0][:100] + "..." if len(t[0]) > 100 else t[0], "count": t[1]}
            for t in trending
        ]

    def start_gpu_session(self, session_id: str):
        """Start tracking a GPU usage session."""
        self.gpu_sessions[session_id] = {"start_time": time.time(), "end_time": None}
