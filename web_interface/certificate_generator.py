...
class CertificateGenerator:
    ...
    def __init__(self, config):
        ...
        self.deployment_mode = (
            getattr(config, "deployment", {}).get("mode", "local")
        ...
        try:
            if not validate_kb_config(config):
                logger.warning(
                    "Knowledge base configuration validation failed. Some features may be limited."
                )
        except Exception as e:
            logger.warning(
                f"Could not validate knowledge base configuration: {e}. Some features may be limited."
            )
    ...
    def _generate_remote(
        ...
        try:
            logger.info(
                f"Generating certificate via API with model: {model_key}, RAG k: {rag_k}"
            )
    ...
    def _generate_local(
        ...
        try:
            logger.info(
                f"Generating certificate locally with model: {model_key}, RAG k: {rag_k}"
            )
            if domain_bounds:
            ...
            if rag_k > 0 and kb_info is not None:
                try:
                    ...
                    context_chunks = (
                        context.count("--- Context Chunk") if context else 0
                    )
                except Exception as e:
                    logger.warning(
                ...
                logger.info(
                    "RAG requested but no knowledge base available. Continuing without context."
                )
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    ...
                    if (
                        ...
                        logger.error(
                            f"Unexpected pipeline result format: {type(result)}"
                        )
                    ...
                    if certificate:
                        logger.info(
                            f"Successfully extracted certificate on attempt {attempt + 1}"
                        )
                        break
                    else:
                        logger.warning(
                            f"Attempt {attempt + 1} failed to extract valid certificate"
                        )

                except Exception as e:
                    logger.error(
                        f"Error during generation attempt {attempt + 1}: {str(e)}"
                    )
