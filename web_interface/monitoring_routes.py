...
def dashboard():
    ...
    if current_user.role not in ["admin", "premium"]:
        ...
        return render_template(
            "monitoring/user_dashboard.html",
    ...
    return jsonify(
        {
            ...
            "metrics": {
                ...
                "success_rate": (
                    (metrics.successful_generations / metrics.total_requests * 100)
    ...
    return jsonify(
        {
            ...
            "metrics": {
                ...
                "api": {"calls": metrics.api_calls, "cost": round(metrics.api_cost, 2)},
                "storage": {
...
def api_system_metrics():
    ...
    return jsonify(
        {
            "cpu": {"percent": round(metrics.cpu_percent, 1)},
            "memory": {
...
def api_certificate_history():
    ...
    return jsonify({"history": history, "count": len(history), "limit": limit})
...
def api_trending_systems():
    ...
    return jsonify({"trending": trending, "period_days": days})
...
def api_export_metrics():
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=metrics_export.{format}"
    )
    ...
    return jsonify(
        {
            "user": {
                ...
                "requests_remaining": user.daily_request_limit
...
def api_v1_history():
    ...
    history = monitoring_service.get_certificate_history(
        user_id=user.id, limit=min(limit, 100)  # Cap at 100
    )
...
def webhook_alert():
    ...
    log_security_event(
        f"monitoring_alert_{alert_type}", severity=severity, description=message
    )
